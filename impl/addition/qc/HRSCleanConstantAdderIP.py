...
class HRSCleanConstantAdderIP(QuantumClassicalAdderIP):
    """
    Constant addition circuit by Häner, Roetteler and Svore (https://arxiv.org/pdf/1611.07995 , Fig 4.)
    """
    ...
    def get_circuit(self, *args) -> QuantumCircuit:
        ...
        # Save an incrementer and 2*n CXs by using clean ancillas.
        # TODO: Maybe make Carry gate a part of CircuitChooser?
        # CARRY Gate
        circuit.append(
            carry_gate(lower_bit_size, c_l, self.c),
            list(self.register_c) +
            list(register_x_l) +
            [self.register_anc[0]] +
            list(register_x_h[:lower_bit_size - 1])  # n - 1 borrowed qubits for carry gate.
        )
        # +1
        controlled_incrementer = CircuitChooser().choose_component(
            "QCIncrementer",
            (higher_bit_size, 0, 1),
            self.dqa + lower_bit_size + self.c,
            # we have the lower bit size as borrowable qubits (and the control qubits)
            self.cqa - 1  # We have one clean qubit less.
        )

        borrowable = list(register_x_l) + list(self.register_g) + list(self.register_c)

        circuit.append(controlled_incrementer.get_circuit(),
                       list([self.register_anc[0]]) + list(register_x_h) + borrowable + list(self.register_anc[1:]))
        # CARRY GATE
        circuit.append(
            carry_gate(lower_bit_size, c_l, self.c),
            list(self.register_c) +
        ...
        low_add = HRSCleanConstantAdderIP(self.dqa, self.cqa, lower_bit_size, c_l, self.c)

        circuit.append(low_add.get_circuit(),
                       list(self.register_c) + list(register_x_l) + list(self.register_g) + list(self.register_anc))

        high_add = HRSCleanConstantAdderIP(self.dqa, self.cqa, higher_bit_size, c_h, self.c)
...
def carry_gate(n, a, c=0) -> Gate:
    ...
    p_bits = [int(i) for i in np.binary_repr(a, n)[::-1]]
    ### Forward passthrough ###
