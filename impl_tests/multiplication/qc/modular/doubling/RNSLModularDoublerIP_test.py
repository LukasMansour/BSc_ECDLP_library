...
class RNSLModularDoublerTests(unittest.TestCase):

    def setUp(self):
        CircuitChooser().clear_caches()

    def test_constant_modular_doubling(self):
        ...
        for p in [7, 9, 11, 15]:
            ...
            for i in range(0, 10):
                ...
                constant_mod_adder = RNSLModularDoublerIP(
                    dirty_anc_available,
                    clean_anc_available,
                    n,
                    p,
                ...
                circuit = QuantumCircuit(register_x, register_g, register_anc, classical_register_x,
                ...
                self.assertEqual(int(key_value[0].split(" ")[2], 2), (x_value * 2) % p)
    ...
    def test_controlled_constant_modular_doubling(self):
        for p in [7, 9, 11, 15]:
            n = math.ceil(math.log2(p))
            for i in range(0, 10):
                # Init
                ...
                circuit = QuantumCircuit(register_c, register_x, register_g, register_anc, classical_register_c,
