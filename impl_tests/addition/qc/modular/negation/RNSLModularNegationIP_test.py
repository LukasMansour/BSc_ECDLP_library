...
class RNSLModularNegationTests(unittest.TestCase):

    def setUp(self):
        CircuitChooser().clear_caches()

    def test_modular_negation(self):
        ...
        for p in [7, 9, 11, 15]:
            ...
            for i in range(0, 10):
                ...
                self.assertEqual(int(key_value[0].split(" ")[2], 2), (p - x_value) % p)
    ...
    def test_controlled_modular_negation(self):
        CircuitChooser()._name_filter = custom_name_filter({"BasicConstantAdderIP", "BasicIncrementer"})
        for p in [7, 9, 11, 15]:
            n = math.ceil(math.log2(p))
            for i in range(0, 10):
